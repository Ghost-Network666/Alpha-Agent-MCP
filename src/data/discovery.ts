...
export function getAgentRecipes(): Record<string, unknown> {
  return {
    ...
    intentRouting: {
      ...
      note: 'Supports 12+ NL queries via intent (e.g. "x sentiment fusion", "use host x_search for sentiment then externalSignals to alpha/strategy", "research categories first (External/Intelligence/Discovery) then execution after signals in strategy", "heartbeat/resource autonomy", "contradiction X vs book"). Host does x_search/sentiment (no native X search in MCP per rules); pass results as externalSignals array to generate_alpha_report/update_strategy. get_strategies() + prompts/get mcp_llms_full_guide first always. For true multi-market autonomy use composite lockedStrategyKey (e.g. "weather:low", "politics:high") in route_agent_intent or run_agent_cycle; the returned plan will include lockedStrategyKey, researchSource, priceMovementCondition, and precise next exact tool+args. Hermes (host) is the brain and owns the heartbeat.md / OpenClaw enforcement loop + primary control; the host (heartbeat-driven) keeps itself locked to that strategy entry and calls MCP planners from its ticks so the MCP remains active as the integration surface providing complete intent routing.',
    ...
    liveDocs: {
      sdkGuide: { prompt: 'mcp_llms_full_guide' },
